...
class PhotosTableViewCell: UITableViewCell {
    ...
    //Переход в галерею с фото
    private let button: UIButton = {
        let button = UIButton()
        button.translatesAutoresizingMaskIntoConstraints = false
        button.tintColor = .black
        button.setImage(UIImage(systemName: "arrow.right"), for: .normal)
        button.addTarget(self, action: #selector(galleryTransition), for: .touchUpInside)
        return button
    }()
    ...
    @objc private func galleryTransition(sender: UIButton) {
    ...
    //Функция для отображения лейбла в ячейке profileViewController для функции cellForRowAt
    ...
    private func setupLayout() {
        ...
        // Отступ в 12 поинтов сверху и снизу
        let indent: CGFloat = 12
        
        // Отступ в 8 поинтов по краям
